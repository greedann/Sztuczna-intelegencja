...
class Node:
    ...
    def gini_best_score(self, y, possible_splits):
        ...
        for idx in possible_splits:
            left = y[:idx + 1]
            right = y[idx + 1:]
            gini_left = 1 - np.sum((np.bincount(left) / len(left)) ** 2)
            gini_right = 1 - np.sum((np.bincount(right) / len(right)) ** 2)
            gain = 1 - (len(left) / len(y) * gini_left + len(right) / len(y) * gini_right)
            if gain > best_gain:
                best_gain = gain
                best_idx = idx
    ...
    def find_best_split(self, X, y, feature_subset):
        ...
        for feature in features:
            ...
            y_sorted = y[order]
            possible_splits = self.find_possible_splits(X[order, feature])
            idx, value = self.gini_best_score(y_sorted, possible_splits)
            if value > best_gain:
                best_gain = value
                best_split = (feature, [idx, idx + 1])
