...
class TorchMultiLayerNetwork(nn.Module):
    ...
    def __init__(self, n_in, n_hiddens, n_out):
        super().__init__()
        self.layers = nn.ModuleList([nn.Linear(n_i, n_o) for n_i, n_o in zip([
                                    n_in] + n_hiddens, n_hiddens + [n_out])])
...
def training(model, x, y):
    ...
    for _ in range(n_steps):
        ...
        if i_step % 1000 == 0:
            print(
                f'Step={i_step}, lr={learning_rate:.6f}, loss={loss.item():.6f}')
...
def classify_spirals(student_id, do_data_inspection=True, do_model_inpection=True, do_model_training=True,
    ...
    if do_data_inspection:
        ...
        plot_data(x, y, x_test=x_test, y_test=y_test,
                  plot_xy_range=[-1.2, 1.2])
    ...
    if do_model_inpection:
        ...
        for name, param in model.named_parameters():
            # czy dla tego parametru będzie potrzebne liczenie gradientu
            print(name, param.requires_grad)
            # aktualna wartość gradientu
            print(name, param.grad)

    print(
        f'Train accuracy: {evaluate_model(model, x_train, y_train) * 100:.02f}%')
    print(
        f'Test accuracy: {evaluate_model(model, x_test, y_test) * 100:.02f}%')
